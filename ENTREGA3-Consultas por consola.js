...
//ventasPorSucursal('2020-06-28', '2020-06-30')
function ventasPorSucursal(fdesde, fhasta) {
    ...
    productos.forEach(function (prod) {
        if (db.venta.find({ "listaItemVenta.producto._id": prod._id }).count() > 0) {
            var itemProducto = db.venta.aggregate([
                ...
                {
                    $match: {
                        "listaItemVenta.producto._id": prod._id, fecha: {
                            $gte: ISODesde,
                            $lt: ISOHasta
                        }
                    }
        ...
        productos.forEach(function (prod) {
            if (db.venta.find({ "listaItemVenta.producto._id": prod._id }).count() > 0) {
                var itemProducto = db.venta.aggregate([
                    ...
                    {
                        $match: {
                            "listaItemVenta.producto._id": prod._id, "sucursal._id": suc._id, fecha: {
                                $gte: ISODesde,
                                $lt: ISOHasta
                            }
                        }
...
//CONSULTA 4:
//ventasPorSucursalYPorTipoProducto('2020-06-28', '2020-06-30')
function ventasPorSucursalYPorTipoProducto(fdesde, fhasta) {
    var ISODesde = ISODate(fdesde);
    var ISOHasta = ISODate(fhasta);
    //Detalle deventas por toda la cadena
    print("------------------------------------------------------------------------------------------");
    print("Ventas para la cadena completa: ");
    var tiposProducto = ["Farmacia", "Perfumeria"];
    tiposProducto.forEach(function (tipo) {
        var productos = db.producto.find({ "tipoProducto": tipo });
        print("------------------------------------------------------------------------------------------");
        print("Productos de " + tipo + ": ");
        print("------------------------------------------------------------------------------------------");
        productos.forEach(function (prod) {
            if (db.venta.find({ "listaItemVenta.producto._id": prod._id }).count() > 0) {
                var itemProducto = db.venta.aggregate([
